...
class CNN1DClassifier(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(64 * 2, 128)  # Conv1d katmanlarından kaynaklanan çıkış boyutu
        self.fc2 = nn.Linear(128, 2)  # İki sınıf için çıkış boyutu
        self.dropout = nn.Dropout(0.25)

    def forward(self, x):
        x = x.unsqueeze(1)  # Giriş boyutuna kanal boyutu ekleyin
        x = F.relu(self.conv1(x))
        x = self.pool(x)
        x = F.relu(self.conv2(x))
        x = self.pool(x)
        x = x.view(-1, 64 * 2)  # Conv1d katmanlarından kaynaklanan çıkış boyutunu yeniden şekillendirme
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = self.fc2(x)
        return F.log_softmax(x, dim=1)

# CUDA aygıtını kontrol etme
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f'CUDA aktif mi: {torch.cuda.is_available()}')

# Örnek giriş tensörü oluşturma
sample_input_cnn = torch.randn(1, 9).to(device)

# CNN1D modeli giriş ve çıkış boyutlarını kontrol etme
model = CNN1DClassifier().to(device)
print(f"CNN1D Modeli Giriş Boyutu: {sample_input_cnn.shape}")
print(f"CNN1D Modeli Çıkış Boyutu: {model(sample_input_cnn).shape}")
