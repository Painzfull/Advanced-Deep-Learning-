...
sample_input = torch.randn(1, 1, 9).to(device)
...
for epoch in range(num_epochs):
    ...
    correct_train = 0
    total_train = 0
    for i in tqdm(range(0, len(X_train), batch_size), desc=f'Epoch {epoch+1}/{num_epochs} - Train', unit=' batches', leave=False):
        optimizer.zero_grad()
        batch_X_train = X_train[i:i+batch_size]
        batch_y_train = y_train[i:i+batch_size]
        output = model(batch_X_train)
        loss_train = criterion(output.unsqueeze(0), batch_y_train)
        loss_train.backward()
        optimizer.step()
        running_loss_train += loss_train.item()
        _, predicted_train = torch.max(output, dim=-1)
    ...
    correct_test = 0
    total_test = 0
    for i in tqdm(range(0, len(X_test), batch_size), desc=f'Epoch {epoch+1}/{num_epochs} - Test', unit=' batches', leave=False):
        batch_X_test = X_test[i:i+batch_size]
        batch_y_test = y_test[i:i+batch_size]
        output = model(batch_X_test)
        loss_test = criterion(output.unsqueeze(0), batch_y_test)
        running_loss_test += loss_test.item()
        _, predicted_test = torch.max(output, dim=-1)
